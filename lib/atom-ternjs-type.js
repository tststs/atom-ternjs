...
class Type {
  ...
  queryType(editor, e) {
    ...
    buffer.backwardsScanInRange(/\]|\[|\(|\)|\,|\{|\}/g, new Range([rowStart, 0], [position.row, position.column]), (obj) => {
      ...
      const scopeDescriptor = editor.scopeDescriptorForBufferPosition([obj.range.start.row, obj.range.start.column]);

      if (
        scopeDescriptor.scopes.includes('string.quoted') ||
        scopeDescriptor.scopes.includes('string.regexp')
      ) {
