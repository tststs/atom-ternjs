...
class Manager {
  ...
  setActiveServerAndClient(uRI) {
    ...
    const paths = atom.project.relativizePath(uRI);
    this.server = this.getServerForProject(paths[0]) || this.getServerForProject(paths[1]);
    this.client = this.getClientForProject(paths[0]) || this.getClientForProject(paths[1]);
    ...
    if (
      ...
      const absolutePath = path.resolve(__dirname, paths[1]);
