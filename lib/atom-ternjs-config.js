...
export default class Config {
  ...
  prepareLibs(configStub) {
    ...
    if (!this.config.libs) {
      ...
      let libsAsObject = {};
      for (let lib of this.config.libs) {
      ...
      this.config.libs = libsAsObject;
    }

    for (let lib of Object.keys(configStub.libs)) {

      if (!this.config.libs[lib]) {
    ...
    for (let lib of Object.keys(this.config.libs)) {
  ...
  preparePlugins(availablePlugins) {
    ...
    for (const plugin of Object.keys(availablePlugins)) {
      ...
      if (this.config.plugins[plugin]) {
        ...
        this.config.plugins[plugin] = this.mergeConfigObjects(availablePlugins[plugin].options, this.config.plugins[plugin]);
        this.config.plugins[plugin]._active = true;

      } else {

        this.config.plugins[plugin] = availablePlugins[plugin].options;
  ...
  buildNewConfig() {
    ...
    if (!_.isEmpty(this.config.plugins)) {
