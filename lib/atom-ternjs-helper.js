...
export function formatTypeCompletion(obj, isInFunDef) {
  ...
  if (obj.rightLabel.startsWith('fn')) {
    ...
    if (
      ...
      if (!isInFunDef) {
        ...
        obj._snippet = params.length ? `${obj.name}(\${${0}:${undefined}})` : `${obj.name}()`;
      }
...
export function getFileContent(filePath) {
  ...
  let projectRoot = manager.server && manager.server.projectDir;
  ...
  const _filePath = projectRoot + filePath;
