...
export function isValidEditor(editor) {
  ...
  const scopeChain = editor.getRootScopeDescriptor().getScopeChain();

  if (/^\.source\.js\b/.test(scopeChain)) {
...
export function buildDisplayText(params, name) {
  ...
  const suggestionParams = params.map(param => {
...
export function buildSnippet(params, name) {
  ...
  const suggestionParams = params.map((param, i) => {
...
export function formatTypeCompletion(obj, isProperty, isObjectKey, isInFunDef) {
  ...
  if (obj.rightLabel.startsWith('fn')) {
    ...
    const params = extractParams(obj.rightLabel);
    const editor = atom.workspace.getActiveTextEditor();
    const cursor = editor.getLastCursor();
    const bufferPosition = cursor.getBufferPosition();
    const bufferPositionFollowing = [bufferPosition.row, bufferPosition.column + 1];
    const charFollowing = editor.getTextInBufferRange([bufferPosition, bufferPositionFollowing]);
    const shouldAddParenthesesToSnippet = charFollowing !== '(';
